...
export class FarmersController {
  ...
  @Post("farms")
  async addFarm(@GetUser() user: User, @Body() farmData: CreateFarmDto) {
  ...
  @Get("farms/:farmId")
  async findFarm(@GetUser() user: User, @Param("farmId") farmId: string) {
    const farmer = await this.farmersService.findByUserId(user.id);
    const farm = await this.farmersService.findFarm(farmId);
  ...
  async findBankAccount(
    ...
    if (bankAccount.farmer_id !== farmer.id) {
      throw new UnauthorizedException(
        "You can only view your own bank accounts",
      );
    }
  ...
  @ApiResponse({
    status: 200,
    description:
      "Returns farmer details with user information for the given offer",
  })
