...
export class BusinessMetricsService extends BaseService<BusinessMetric> {
  ...
  async getErrorRateByPath(
    ...
    const results = await this.businessMetricRepository
      ...
      .addSelect(
        "COUNT(*) FILTER (WHERE metric.status = :errorStatus)",
        "errors",
      )
  ...
  async getResponseTimePercentiles(
    ...
    const results = await this.businessMetricRepository
      ...
      .select("metric.path", "path")
      .addSelect(
        "PERCENTILE_CONT(0.5) WITHIN GROUP (ORDER BY metric.response_time)",
        "p50",
      )
      .addSelect(
